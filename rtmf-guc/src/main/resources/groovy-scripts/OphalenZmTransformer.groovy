...
if (payload.contains("ophalenMeldingStatus")){
	...
	meldingKenmerk = meldingKenmerk.substring(meldingKenmerk.lastIndexOf(" - (K:") + 6, meldingKenmerk.length()-1)
	result = String.format(baseZaakDetailRequestPayload, meldingKenmerk)
} else if (payload.contains("ZaakDetailRequest")) {
	return payload
}
...
def transformDatum(inDatum) {
	...
	if (!isEmptyOrNull(inDatum)) {
		result = inDatum.substring(0,10) + 'T23:59:00.000'
	}
	return result
}
